...
学生账号由教师在班级学生列表中开通，登录账号固定取规范化后的学生学号，初始密码为教师设置的一次性临时密码；首次登录必须改为至少 12 位的新密码。教师仍使用邮箱登录。学生账号的安全邮箱为选填项，与教师维护的学生档案联系邮箱相互独立；学生登录后可在“安全邮箱设置”中新增、更换或清除，变更时必须再次输入当前密码，变更后原验证状态及未使用验证码立即失效。安全邮箱经学生本人验证码确认后，可在 `/forgot-password` 使用“学号 + 安全邮箱 + 一次性验证码”重置密码；未设置或未验证安全邮箱的学生仍可正常学习，但不能使用邮件自助找回密码。重置成功会撤销该账号的全部旧会话。验证码只保存 HMAC 摘要，默认 10 分钟失效、最多尝试 5 次。不同教师名下的学号若规范化后相同，系统会拒绝开通第二个账号，避免登录身份歧义。

开发环境未配置 SMTP 时，界面会显示仅用于本机验收的开发验证码。生产环境必须配置加密 SMTP、发件邮箱和独立的 `AUTH_RECOVERY_HMAC_SECRET`；生产配置校验会拒绝无 SMTP、无加密或复用会话密钥的启动。
