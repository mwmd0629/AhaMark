...
5. 对分式、根号、上下标、极限下标、积分、求和、矩阵或分段函数，切换到“公式区域”，逐个框住完整公式。优先在公式卡片的“普通数学输入”中使用下方有限语法，生成 LaTeX 和线性替代文本草稿，再查看完全离线的结构预览。生成结果固定保持“待核对”，只有人工点击“与原图一致”才会改为已核对；编辑普通输入、LaTeX 或线性替代文本都会自动撤销已核对状态。原始 LaTeX 默认折叠在“高级编辑”中。
...
工具栏的“分式、根号、上标、下标、极限、积分、求和、矩阵、分段函数”按钮只插入上述可编辑模板。嵌套结构必须逐层使用括号，例如 `[sqrt([a]/[b])+x^2]/(y_(i+1))`。连续裸除法、缺括号、空上下标、不等长矩阵、缺条件的分段函数、不支持字符或无法唯一确定的结构都会停止转换并显示位置，不会猜测。用户可以选择“需要修改”展开高级编辑，或选择“需要辅助转写”继续保持 pending；只有原图局部确实无法可靠辨认时才使用 `unreadable`，不把整页改为 `unjudgeable`。
