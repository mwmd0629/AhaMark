...
4. 对照图片修正 OCR 草稿的错字、漏字、数学符号、换行和阅读顺序，再勾选“已逐字核对正文”。任何后续正文编辑都会自动取消该勾选。
5. 对每页填写题号、题目区域、内容标签、退化标签和完整性决定。
6. 若页面可见姓名、学号、邮箱、电话等身份信息，先在独立图片副本中遮挡并替换该 UUID PNG；工具本身不会猜测或自动擦除身份信息。
7. 只有正文核对、标注状态和隐私复核都完成后，页面才计为完成。
